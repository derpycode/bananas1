...
\noindent will turn the vector \texttt{B} into one (assigned to the \textit{variable} \texttt{D}) with he same orientation as \texttt{C}. \sidenote{Note ... \textbf{MATLAB} gives the syntax as \texttt{.'}, whereas I always only ever added the \texttt{'} bit ... which works ...}
...
\\ \ \ squares');   
\end{docspecmargin}
(The notation gets confusing in a narrow box -- the\texttt{ ...} indicates that the line should be continuous and not broken across 2 lines.)
\end{mdframed}}
...
It is unlikely that you want to have to save all your files here. More likely, you may have a course folder somewhere, possibly with sub-folders for each week (or whatever), so you will need to change the \textbf{MATLAB} directory that you are working in, to match where your files are. At the command line -- you can change the directory that \textbf{MATLAB} is working from by typing:
\begin{docspec}
>> cd DIRECTORY\_PATH
\end{docspec}
...
Now, import the data into the \textbf{MATLAB} workspace using the  \texttt{load} command. Because the data is a plain text (ASCII) format and not a special \textbf{MATLAB} \textsf{.mat} file, you need to specify the format as '\texttt{ascii}' when using the \texttt{load} command (see box or \textsf{help}):

\begin{docspec}
\textbf{>> load('etheridge\_etal\_1996.txt','-ascii');}
\end{docspec}
...
\noindent So you need to specify each column of the data (i.e. each \textit{vector}) separately and explicitly.\footnote[][-2.5in]{If you just type \texttt{plot} and pass the (here: default) name of the data array: 
\begin{docspecmargin}
...
\noindent (Labelled as: 'Observed global mean temperature anomaly \uline{data}'.)
...
\noindent but instead, must pass the 2 vectors (of x, and y-axis data), seperately:
...
\docenv{errorbar} -- see \textbf{MATLAB} \textsf{help}.

Or  one could plot the min and max values as points, using different symbols. This requires a further little trick in \textbf{MATLAB}, which involves the command \docenv{hold}. This is nice and simple and takes the additional (2nd) word: \texttt{on}, or \texttt{off}.
