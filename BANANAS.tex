...
\noindent(where \texttt{X} is the name of your first vector, and \texttt{Y} is the name of your second).
 ...
 Do this, and depending on just what or how random your \textit{y}-axis data was, you should end up with something like Figure \ref{fig:plot-basic} in a window captioned "\texttt{Figure 1}"\sidenote{If you cannot see the figure window ... check that the window is not hidden behind the main \textbf{MATLAB} program window!} (although your actual line is likely to look very different from this depending on what vectors you created).
...
\marginnote[-2.25in]{\begin{mdframed}[backgroundcolor=gray!10, roundcorner=10pt,leftmargin=1, rightmargin=1, innerleftmargin=5, innerrightmargin=5, innertopmargin=5,innerbottommargin=5, outerlinewidth=1, linecolor=light-gray]
\docenvdef{axis}
...
To start with: multiply all the elements of \texttt{A} by the number \texttt{17}. Assign the answer to a 3rd array (\texttt{C}). What is the sum of the 2nd row of \texttt{C}?
...
There are a number of different ways to load/import data into the \textbf{MATLAB} \textsf{Workspace}. Rather than try and tediously list and describe in detail in this section, all the commands and syntax and blah blah, we'll take a simple over-view and then go through some more detailed and data-based examples as we progress through the course text.
...
\textbf{MATLAB} has a default directory that it starts up in and looks at first. For basic \textbf{Windoz} installations\sidenote[][-1.0in]{At installation, this directory can be specified and hence may not be this one. Also -- different operating systems will have different default locations.} of the software, this directory might be:
\vspace{1mm}
...
> cd
\end{docspec}
\noindent Your current directory should also be displayed in a toolbar above the \textsf{Command Window}. 
...
\noindent You can also do this (move around directories) at the the command line ... which is much less fun and you are less likely to need to do it this way. The instructions are included below for completeness ... but you can skip over them if you wish
...
As a brief exercise and practice in using \texttt{load} -- first download the data file \texttt{etheridge\_etal\_1996.txt} from the course webpage. Note that you need to either save this file directly in the folder you intend to work from (and which you have directed \textbf{MATLAB} to by changing its working directory), or copy the file from your computers 'Download' folder if it ends up there, to the working directory.\footnote{In eLearn -- click on the file to take you to a page where the contents are displayed. \\At the top of this page, you should see: \textsf{Download etheridge\_etal\_1996.txt (5.27 KB)} with the filename highlighted. Right-click over the link, and choose \textsf{Save link as ...} make sur eyou are saving as a \textsf{.txt} text file and not as a 'web page'.}
...
\noindent Now, import the data into the \textbf{MATLAB} workspace using the  \texttt{load} command.  This looks like (see \texttt{help} on the command \texttt{load}, or the margin help box in thjis document):
