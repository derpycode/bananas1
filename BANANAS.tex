...
Conversely, there are also good jobs and  \$\$\$ in being able to program. So, maybe stick with the course and read on ...
...
So far, most of your \textit{variables} have all been what are known as \textit{scalars} -- i.e. single numbers (whether \textit{real} or \textit{integer})\sidenote{An exception are when you assigned a string, which technically is a vector (assuming multiple characters in the string)}.
...
X = [1:10];
\end{docspec}
Or, combining with the exponentation arithmetic \textit{operators} (see earlier):
\begin{docspec}
Y = [1:10].\(^{\wedge}\);
\end{docspec}
...
\noindent In the following, having previously created the matrix \texttt{A}, you will derive a matrix \texttt{C}, and then preform some operations on it. 

To start with: multiply all the elements of \texttt{A} by the number \texttt{17} and assign the answer to a 3rd array (\texttt{C}).\sidenote[][-0.0in]{You should be typing in something like this:
\begin{docspecmargin}
\textbf{>> C = 17*A;}
\end{docspecmargin}}


Now: what is the sum of the 2nd row of \texttt{C}?
