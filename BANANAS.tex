...
Consider, or rather: create a new \textit{script} \textsf{m-file}\sidenote{Comment it!}, and add following  code for a simple loop:
\begin{docspec}
for n=1:10
\\end
\end{docspec}
...
\item In the \textbf{MATLAB} \textsf{Editor}, add a \textit{breakpoint} to the \texttt{end} line of the loop. When you run the script, the program stops after the first time around the loop (and the \texttt{end}). You can now e.g. interrogate the value of n by hovering your mouse pointer over it. You can also \textsf{Continue} (button in the icon bar) the program, and it will then go around the loop once more and then stop again at end. 
