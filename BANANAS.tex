...
\item Color highlights in the text are used to mirror the colors employed by \textbf{MATLAB} at the command line, or in the code editor.

\item Math is highlighted in a  different font, e.g:
...
\vspace{4mm}
\noindent \uline{Note that for now (this Chapter), you will only be entering code at the \textit{Command line} (not in a file, which will come later in Chapter 2)}.
...
Conversely, there are also good jobs if you are able to program ... so maybe stick with the course and read on ...

%------------------------------------------------

\subsection{The command line\index{The command line}}

When \textbf{MATLAB} initially starts up, the \textit{Command Window}\index{Command Window} should be blank except for a vertical blinking line (cursor) following the double 'greater than' symbols with an ocean of blank lines/space below\sidenote{Note that in nerd-speak the \texttt{>>} is called the command 'prompt' and is prompting you to type some input (Commands, swear words, etc.). See -- the computer is just sat there waiting for you to command it to go do something (stupid?). If one does not appear at the bottom of whatever is in the \textit{Command Window} is means that \textbf{MATLAB} is busy doing something extremely important. Or perhaps, \textbf{MATLAB} may have completely died. Either way, it will not accept any new/further commands until it is done calculating/dying.}\sidenote{Older versions of \textbf{MATLAB} might have displayed: \begin{docspecmargin}
        Academic License
...
\noindent \uline{Pause} ... this (what you have just done here) is sort of fundamental (to using \textbf{MATLAB}). It is the equivalent of typing '10' into the cell A1 in Excel (assuming we can equate the \textbf{Excel} location \textsf{A1} with the \textbf{MATLAB} variable \texttt{A}). In doing this, you have both: (a) created a variable \texttt{A}, and (b) assigned it a value of \texttt{10}.
...
\noindent \uline{Pausing again} ... if you want to remind yourself of the \textit{variables} that you (or a program) have created -- you can refer to the \textsf{Workspace} window.\sidenote{There is a command line command for listing current variables (\texttt{whos}), but lets not bother with it.} Also listed here as noted above, is its value (and \textit{type} etc). Another way to access the value of a \textit{variable}, is to simply type in its name at the command line:
...
\textbf{MATLAB} also provides several built-in mathematical \textit{constants} (which save having to define a variable with the appropriate number that you no-doubt will have to look up from the internet first ...). This are simply \textit{variables} that have been already defined and assigned values, but which  you cannot change (hence 'constant'). For instance,  the value of \(\pi\) is assigned to a built-in \textit{function} with the name \docenvdef{pi}. You can recover  its value  by typing its name at the command line:
...
\item \docenvdef{clear} --- Removes all variables from the workspace.
 \item \docenvdef{close} --- Closes the current figure window.
...
The \textit{variables} that you have seen so far are known as \textit{scalars} -- i.e. single numbers (whether \textit{real} or \textit{integer})\sidenote{An exception are when you assigned a string, which technically is a vector (assuming multiple characters in the string)}.
One of the most powerful things about \textbf{MATLAB} is its ability to represent vectors (1D columns or rows of numbers or characters) and arrays -- 2D and higher dimensional regular grids of numbers or strings. (\textit{matrix}\sidenote{Not to be confused with the film containing bad acting by Keanu Reeves.} is the name commonly given to a 2-D array.)
...
\textit{Vectors} are 1-D arrangements of numbers (or characters or \textit{strings}).  You can enter them into \textbf{MATLAB} as a list of space-separated values, encased in (square) brackets, \texttt{[ ]}, e.g. 

\begin{docspec}
B = [0.5 1.0 1.5 2.0 2.5]
\end{docspec}

\noindent or with the values comma-separated:
 
\begin{docspec}
B = [0.5, 1.0, 1.5, 2.0, 2.5]
\end{docspec}

Either way, you end up with a \textit{row vector} on its side as a single row of numbers which in math-speak would look like:
...
You can also create the equivalent, upright orientated \textit{column vector} (as a single column of numbers) by separating the elements by a semi-colon:
...
\noindent will turn the vector \texttt{B} into one (assigned back to the same \textit{variable} name) with the same orientation as \texttt{C}. \sidenote{Note ... \textbf{MATLAB} gives the syntax as \texttt{.'}, whereas I always only ever added the \texttt{'} bit ... which works ...}
...
\subsection{Using the colon operator to create vectors}

Refer to the Box on the use of the \docenv{colon operator}. But basically, to create a vector of all the integers from \(1\) to \(10\):
\begin{docspec}
X = [1:10];
\end{docspec}
...
\marginnote[-4.125in]{\begin{mdframed}[backgroundcolor=light-gray, roundcorner=10pt,leftmargin=1, rightmargin=1, innerleftmargin=5, innerrightmargin=5, innertopmargin=5,innerbottommargin=5, outerlinewidth=1, linecolor=light-gray]
...
\noindent(In this text, I will refer to accessing a particular element (or elements) of a \textit{vector} (or \textit{array}) via its \textit{index} as \textit{addressing}. Unless I forget, then I might say something else. You'll have to keep on your toes -- don't expect consistency here!)\sidenote[][-1.25in]{Recognise the parallel with \textbf{Excel} here -- the value in position \texttt{5} in the \textbf{MATLAB} vector \texttt{B}, is the same as specifying the contents of cell \textsf{A5} in \textbf{Excel}.}

\vspace{1mm}
\noindent\rule{4cm}{0.5pt}
\vspace{2mm}

\noindent There is a \textbf{MATLAB} \textit{function} \docenv{end} (see Box) that enables you to easily address (accessing via its index) the very last value in a \textit{vector} (in \textbf{MATLAB}, the \textit{index} of the first position is always \texttt{1}).

For addressing more than one element of a vector at a time, you can use the \docenv{colon operator} (see Box). \sidenote[][-1.75in]{Again -- e.g. in \textbf{Excel}, the sum of the \textsf{5} elements in column \textsf{A} (the equivalent 'vector'), would be \textsf{=SUM(A1:A5)}.}
...
\marginnote[-3.00in]{\begin{mdframed}[backgroundcolor=light-gray, roundcorner=10pt,leftmargin=1, rightmargin=1, innerleftmargin=5, innerrightmargin=5, innertopmargin=5,innerbottommargin=5, outerlinewidth=1, linecolor=light-gray]
...
First -- you will create yourself a dummy dataset to plot. To do this, you are going to need to create yourself a pair of \textit{vectors} -- these can have any values in them that you like, but perhaps aim for 1 vector with values counting up from 1 to 10 (or similar) -- this will form your \textit{x}-axis, and the 2nd column (the second vector you will create) ... whatever you like. \sidenote[][-3.00in]{Looking ahead -- you could create a y-axis \textit{vector} formed of the squares of the numbers in the x-axis \textit{vector}: 
\begin{docspecmargin}
>> Y = X.\(^{\wedge}\)2  
\end{docspecmargin}
...
With any new \textbf{MATLAB} command (\textit{function}), get into the habit of looking up the \textsf{help} text:
\begin{docspec}
>> help plot
\end{docspec}

Note that you may prefer the formatted on-line version -- go to \href{https://www.mathworks.com/help/matlab/}{https://www.mathworks.com/help/matlab/} and type \textsf{help} in the search box. (Don't forget that you can also refer to alternative/simplified help provided in the left-hand margin of this text.) 
...
\noindent Do this. Depending on just what or how random your \textit{y}-axis data was, you should end up with something like Figure \ref{fig:plot-basic} in a window captioned "\texttt{Figure 1}"\sidenote[][-0.375in]{If you cannot see the figure window ... check that the window is not hidden behind the main \textbf{MATLAB} program window!} (although your actual line is likely to look very different from this depending on what vectors you created).
...
\begin{enumerate}[noitemsep]
\setlength{\itemindent}{.2in}
\item The first, obvious thing to do is to add axis labels. The commands are simple -- \docenvdef{xlabel} and \docenvdef{ylabel}. They each take a string as an input, which is the text you would like to appear on the axis (see Box). If you change your mind, simply re-issue the command with the text you would like instead. For example:
\begin{docspec}
>> xlabel('This is the x-axis.');\\
>> ylabel('This is the y-axis.');
\end{docspec}
\vspace{1mm}
\item The command for title, perhaps unsurprisingly, is \docenvdef{title} (also see Box). Again, pass the text you would like to appear as a string (in inverted commas \texttt{' '}), or pass a the name of variable that contains a string (no \texttt{'}' is then needed). e.g.:
\begin{docspec}
>> title('This is a pointless graph the Instructor made us plot.');
\end{docspec}
\vspace{1mm}
\item You might want to specify the axis limits. The command is \docenv{axis} (see Box) and it takes a vector of 4 values as its input -- in order: minimum x, maximum x, minimum y, and maximum y value. e.g.:
\begin{docspec}
>> axis([0 10 -100 100]);
\end{docspec}
would specify an \textit{x}-axis running from 0 to 10, and a \textit{y}-axis from -100 to 100.
\end{enumerate}
...
\marginnote[-3.625in]{\begin{mdframed}[backgroundcolor=light-gray, roundcorner=10pt,leftmargin=1, rightmargin=1, innerleftmargin=5, innerrightmargin=5, innertopmargin=5,innerbottommargin=5, outerlinewidth=1, linecolor=light-gray]
...
\\>> y2 = sin(2*x);}
\end{docspec}
(Note how in the first line, the \textit{colon operator} is used to create an \textit{x} vector from 0 to 2\(\pi\), in steps of 0.1. The second and third lines calculate the sine of all the x values, and sine of 2 times the \textit{x} values, respectively, and assign the results to vectors, \texttt{y} and \texttt{y2}.)
...
\end{itemize}
\item Load matrices from an external data file.
...
You can treat \textit{vectors} and \textit{matrices} (or parts of \textit{vectors} and \textit{matrices}), mathematically, as you would treat single values (i.e. \textit{scalars}) but unlike a \textit{scalar}, the transformation is applied to all specified elements of the \textit{array}. This applies for all the basic \textit{arithmetic operators}\sidenote{Technically ... or at least to be consistent with other operations, you might write multiplication as \texttt{.*} rather than just plain old \texttt{*}. The preceding dot tells \textbf{MATLAB} not to treat this as matrix multiplication but to carry out the operation on each element in turn. In this case, it is the same thing (and both notations work the same), but later, is not. (This will make more sense when you get to see it in action, later.)}. For example, for \textit{array} \texttt{A},
\begin{docspec}
>> 2*A
\\ans =
\\\ \ \ \ 30  14  22  12\\
...
Additional matrix operations you will find useful include the \docenv{transpose} function (\texttt{'}) which flips a (2D) \textit{matrix} around its leading diagonal (columns become rows, and rows, columns)\sidenote[][-1.0in]{This is almost true. Technically the function you want is \texttt{.'}, as \texttt{'} will change the sign of any imaginary components. For real numbers, they are the same.}. Also \docenv{flipup} (e.g., >> \texttt{flipup(A)}) and \docenv{fliplr} (e.g., >> \texttt{fliplr(A)}).
...
Try out these three operators on your matrix \texttt{A}.

\noindent Lastly -- earlier you saw how to find the length and size of a \textit{vector} and \textit{array}. Determining the sum of all the elements also comes in handy. For a vector this is straightforward, and
\begin{docspec}
>> sum(B)
\\ans = 7.5
\end{docspec}
(assuming that you did not alter any of the values in \texttt{B} earlier).

For an \textit{array} (if you check out \textbf{MATLAB} help on \docenv{sum}) it is a little more complicated. As per detailed in help, "If A is a matrix, then sum(A) returns a row vector containing the sum of each column." If you want the sum of the entire matrix (not just the sums of each column), you have to request:
\begin{docspec}
>> sum(A,"all")
\end{docspec} 
...
\textbf{MATLAB} has a default directory that it starts up in and looks at first. For basic \textbf{Windoz} installations\sidenote[][-2.5in]{At installation, this directory can be specified and hence may not be this one. Also -- different operating systems will have different default locations.} of the software, this directory might be:
\vspace{1mm}
...
You can also do this (move around directories) at the the command line ... which is much less fun and you are less likely to need to do it this way. \uline{The instructions for managing working and file locations at the command line are included in the margin ... for completeness ...  you can skip over them if you wish
\sidenote[][-7.0in]{Just remember that there are ways of navigating around your computer storage via the command line.} ...}

\marginnote[-6.50in]{\begin{mdframed}[backgroundcolor=light-gray, roundcorner=10pt,leftmargin=1, rightmargin=1, innerleftmargin=5, innerrightmargin=5, innertopmargin=5,innerbottommargin=5, outerlinewidth=1, linecolor=light-gray]{
\noindent You can change the directory that \textbf{MATLAB} is working from by typing:
\begin{docspecmargin}
>> cd DIRECTORY\_PATH
\end{docspecmargin}
...
\noindent Another alternative is to add a 'search path' (\texttt{addpath}) so that \textbf{MATLAB} knows of an additional place to look for files.  For example:
\begin{docspecmargin}
...
\marginnote[-0.125in]{\begin{mdframed}[backgroundcolor=light-gray, roundcorner=10pt,leftmargin=1, rightmargin=1, innerleftmargin=5, innerrightmargin=5, innertopmargin=5,innerbottommargin=5, outerlinewidth=1, linecolor=light-gray]
\docenvdef{load}
...
The simplest way to import data into \textbf{MATLAB} is also to use the \textit{GUI} -- from the \texttt{File} menu, selecting the option \texttt{Import Data..}. will run the data import Wizard -- note that you might have to select \texttt{All Files (*.*)} from the file type option box in order to find the file. I'll leave you to work the rest out for yourselves ... The \textit{GUI} can also be used to change the directory you are working from (duplicating the functionality of the \texttt{cd} command) and add paths to search (duplicating the functionality of the \texttt{addpath} command).

A more flexible way that you can embed in programs, is to use the \docenv{load} function (see Box) -- we'll concentrate on this  in the course.

As a brief exercise and practice in using \texttt{load} --  download the data file \texttt{etheridge\_etal\_1996.txt} from the course webpage. Note that you need to either save this file directly in the folder you intend to work from (and which you have directed \textbf{MATLAB} to by changing its working directory), or copy the file from your computers 'Download' folder if it ends up there, to the working directory.\footnote[][0.0in]{In eLearn -- click on the file to take you to a page where the contents are displayed. \\At the top of this page, you should see: \textsf{Download etheridge\_etal\_1996.txt (5.27 KB)} with the filename highlighted. Right-click over the link, and choose \textsf{Save link as ...} make sure you are saving as a \textsf{.txt} text file and not as a 'web page'.}
 
You might start by viewing the contents of the file by opening it in any text viewer or importing it into \textbf{Excel}. This is always a good place to start when importing and processing data in any programming language, as it enables you to see what you are getting yourself in to (i.e. the format of the file, any potential formatting issues, approximate size and complexity of the dataset, etc).

\vspace{1mm}
\noindent\rule{4cm}{0.5pt}
\vspace{2mm}

\noindent Now, import the data into the \textbf{MATLAB} workspace using the  \texttt{load} command.  This looks like:

\begin{docspec}
\textbf{>> load('etheridge\_etal\_1996.txt');}
\end{docspec}

If you tell \textbf{MATLAB} nothing different, it will create a variable for you containing the file contents, with the variable name based on the filename (minus the extension). If you prefer a different variable name, then simply pass the results of the load command -- the contents of your file -- to a different \textit{variable}, e.g.

\begin{docspec}
>> MYDATA = load('etheridge\_etal\_1996.txt');
\end{docspec}

\noindent is exactly the same as before, but  assigns the \texttt{load}-ed data to the \textit{variable} \texttt{MYDATA}\sidenote[][-0.0in]{Remember that all capital letters words indicate a place-holder for your own, here: variable name.} rather than to an automatically-generated one.
...
For practice, try plotting the data\sidenote[][-0.0in]{using \texttt{plot}} and remembering to label the figure appropriately\sidenote[][-0.0in]{ FYI: the first column of the data and x-axis is year, and the 2nd column of the data and y-axis is the mixing ratio of CO\(_{2}\) in air in units of ppm.}. \uline{However} ... remember, the format of the \textbf{MATLAB} \texttt{plot} function is:

\begin{docspec}
plot(X,Y) plots vector Y versus vector X
\end{docspec}

\noindent so you will need to specify each column of the data (i.e. each \textit{vector}) separately and explicitly because the data you loaded is in the form of a single variable containing a 2D array of values.\footnote[][0.0in]{If you just type \texttt{plot} and pass the (here: default) name of the data array: 
\vspace{-4mm}
\begin{docspecmargin}
...
>> save(FILENAME)
\end{docspecmargin}
which saves the entire workspace to a \textsf{.mat} file (with the filename given by the string \texttt{FILENAME} (in quotation marks).\\ In contrast:
\vspace{-4mm}
\begin{docspecmargin}
...
As an example to kick-off some data-processing tricks, load in the dataset of ('proxy') reconstructed atmospheric CO\(_{2}\) concentrations spanning the Phanerozoic: \textsf{paleo\_CO2\_data.txt}. You can just import it into \textbf{MATLAB} using the \docenv{load} function as before -- remember the specific syntax of load:
...
If you tell \textbf{MATLAB} nothing different (and do not assign the results of the \texttt{load} function to a different \textit{variable} name), \textbf{MATLAB} will again automatically create a \textit{variable} called \textsf{paleo\_CO2\_data} (see \textsf{Workspace}) and assigned the loaded data into that.
...
\begin{docspec}
\textbf{>> sorted\_data = sortrows(paleo\_CO2\_data);}
\end{docspec}
...
\begin{docspec}
\textbf{>> plot(sorted\_data(:,1),sorted\_data(:,2));}
\end{docspec}
...
The columns are: (1) year, (2) annual mean ocean+land surface temperature \uline{anomaly} (i.e. temperature change relate to some reference value, where here is the observed 1901-2000 mean). Remember, that you can \texttt{load} and assign data to an easier-to remember variable by e.g.:
...
\noindent \uline{but instead}, must pass the 2 vectors (of x, and y-axis data), separately, i.e.:
...
For this, you will need to take your data (which is the 2nd column of the array), e.g. \texttt{data1(:,2)}, multilpy it by 1.8 as per the equation (\texttt{1.8*data1(:,2)}) and then add 32 to it (i.e. \texttt{1.8*data1(:,2) + 32})\footnote{If you have any doubts as to the order in which the operators are applied, add a set of parentheses, e.g. \\ \texttt{(1.8*data1(:,2)) + 32}}. And ... assign the results of this to a new vector or array. Or you can replace the original column (if you are feeling totally confident), e.g.
...
\textbf{\texttt{plot} options}
\\\noindent The main (i.e. not an exhaustive list) data display options for the \texttt{plot} function are:
\\(1) point style
...
\marginnote[-1.675in]{\begin{mdframed}[backgroundcolor=light-gray, roundcorner=10pt,leftmargin=1, rightmargin=1, innerleftmargin=5, innerrightmargin=5, innertopmargin=5,innerbottommargin=5, outerlinewidth=1, linecolor=light-gray]
\docenvdef{legend}
\\According to \textbf{MATLAB} \textsf{help}:
...
As something (2D data) to play with -- \texttt{load} in the data matrix: \texttt{model\_grid.txt} (labelled as: '2D model grid \uline{data}'). Then, view the data in the array viewer, just to get a feel for what you are dealing with here (although you are unlikely to be much wiser after doing so). 

Now, employ the \docenv{pcolor} function in its simplest possible usage (see Box) to visualize the data -- i.e. you simply pass the name of the variable containing the data matrix into the function. You can see (Figure \ref{fig:ch1-plot-pcolor1} ) that it is ... something. Maybe a little like the continents, but up-side-down at the very least. What to do?
...
Next try something very similar, but use the \docenv{image} function --  refer to the help box but note that for viewing a data matrix its usage is the same as that for \docenv{pcolor}  (you pass the variable name containing the 2D data).\footnote{Now the model grid is the correct way around! I have absolutely no idea why and why it is reading the matrix dimensions differently from \docenv{pcolor}. I am sure you could \textbf{Google} and find out. But you would have to actually care first.} 
...
As your very last exercise -- find an image on the internet that amuses you, download it, load it into \textbf{MATLAB} (using \docenv{imread} -- see Box), visualize it using \texttt{image} (passing the name of the variable you created using  \docenv{imread}), and then ... well, that depends on how amusing it is. Maybe try plotting something on top of it (using \texttt{hold on}) or simply go home.
...
\\In general, you can  pass the name of \uline{any} variable
\begin{docspecmargin}
disp(VARIABLE)
\end{docspecmargin}
...
\noindent OK -- you are now ready for your very first program ... inevitably ... this has to be to print '\texttt{Hello, World}' to the screen. No, really. (\textbf{Google} it.) 

Create a new \textit{m-file}, calling it e.g. \textsf{hello\_world.m} (remembering that spaces are NOT allowed in filenames). Make sure that it is saved in the same directory  that \textbf{MATLAB} is working from ... 

You are going to use the function \docenv{disp} (see margin help box and/or type \texttt{>> help disp}  to find out the \textbf{MATLAB} \textit{function} syntax and usage). This command (/\textit{function})  will print to the screen, either any text you specify (in inverted commas), or the contents of any\textit{} \textit{variable} (you pass the variable name to \texttt{disp} (without inverted commas)). For now, simply pass the text directly.
...
\vspace{1mm}
\item If using a \textit{script} to do some plotting, sometimes (but not always) it is convenient to add at the top of the \textit{m-file}, \begin{docspec}
close all;
\end{docspec}
...
\marginnote[-4.5in]{\begin{mdframed}[backgroundcolor=light-gray, roundcorner=10pt,leftmargin=1, rightmargin=1, innerleftmargin=5, innerrightmargin=5, innertopmargin=5,innerbottommargin=5, outerlinewidth=1, linecolor=light-gray]
...
\\...
\end{docspec}
where \texttt{happy} is a variable which is  \texttt{true} (\texttt{1}) or \texttt{false} (\texttt{0}).
\vspace{1mm}
...
\vspace{1mm}
\item The result of a \textit{relational operator} (see earlier), i.e. one of e.g.:
\begin{docspec}
>, <, <=, >=, ==, \(\sim\)=, \&\&, ||
\end{docspec}
 applied to a pair of \textit{variables}, one \textit{variable} and one value, or two values, e.g.:
\begin{docspec}
if A > B
\\...
\end{docspec}
...
\newthought{A rather computer programming textbook-like example} now follows ... :(

We will create a program (a \textbf{MATLAB} \textit{script} saved as an \textit{m-file})  that asks whether or not you like bananas, and if you answer 'yes', tells you 'Correct -- they are a great fruit!'(!)
...
Secondly, (and on the next line after the comment line) -- define the text (question) that you are going to ask and assign this string to the variable \texttt{MY\_QUESTION} (substitute your own variable name here). 
...
You should now have a program consisting of 3 (or more, depending on how many \textit{comment} lines you include) lines -- an initial \textit{comment} line, a line defining the question and assigning this string to a handy variable (\texttt{MY\_QUESTION}), and a line taking the results of the \texttt{input} function, and assigning it to a second variable (\texttt{MY\_ANSWER}). The structure of your program should look like Figure \ref{fig:ch2-bananas}. To help you out, a complete program looks like:
...
You are going to add in an '\texttt{if ...}' statement to the code (starting on a line at the end of your current code) to test whether the answer, held in the variable \texttt{MY\_ANSWER}, is equal to '\texttt{yes}'.
...
\noindent where \texttt{[RESPONSE]} you will later replace by a message that you will display using the \texttt{disp} command that you saw before. (In this stupid example it might be: 'Correct -- they are a great fruit!').
...
\noindent Next, you might display an alternative message is the answer is not 'yes'. Refer to \textbf{help} / the margin Box on \texttt{if ...} and note that you can extend the structure with an \texttt{else} which would be followed by a line displaying the alternative message (e.g. 'Then you need to get a life, apple-lover.')\sidenote[][-0.75in]{And then the line with \texttt{end} after that -- follow the prescribed structure *exactly*.}.
...
sum(y(:));
\end{docspecmargin}
(Both give the sum of all the elements in \texttt{y}.)
\end{enumerate} 
}
...
\uline{Create a new \textbf{m-file}} and enter the following code:
...
\marginnote[-0.125in]{\begin{mdframed}[backgroundcolor=gray!10, roundcorner=10pt,leftmargin=1, rightmargin=1, innerleftmargin=5, innerrightmargin=5, innertopmargin=5,innerbottommargin=5, outerlinewidth=1, linecolor=gray!10]
\docenvdef{num2str}
...
\item \textbf{Using \docenv{ncread}, which reads data directly from the file.}
...
\subsection{Importing ... with \textbf{Import Data}}

Finally, there is also a GUI icon \textsf{Import Data} and a little like Excel and gives you the option to specify the number of header lines to ignore etc. Try playing with this for any of the files imported in this section (except the netCDF one).
...
\begin{docspec}
etopo1deg\_max=max(max(etopo1deg));
\end{docspec}
...
\begin{docspecmargin}
h.Label.String = 'The units of my lovely colorbar';
\end{docspecmargin}
...
Y2 = [50 0];
\end{docspec}
where \texttt{X2} contains the 2 x-coordinate values, and \texttt{Y2} contains the 2 y-coordinate values, such that you are plotting a line between (\texttt{X2(1)},\texttt{Y2(1)}) and (\texttt{X2(2)},\texttt{Y2(2)}).
...
What this does is to tell \textbf{MATLAB}: take the graphics object with the \textbf{ID} contained in variable \texttt{h} (which we just retried via the \texttt{gca} function), and set (which is why we use the command \texttt{set} ...) the following properties:
\begin{itemize}[noitemsep]
\setlength{\itemindent}{.2in}
...
\end{docspecbold}
Here, \texttt{'Position',[0 0 1 1]} specifies that the axes area should fill the window, and \texttt{'Visible','off'} says to make the axes invisible. (We keep a copy of the \textit{handle} variable, \texttt{h}, just in case we need it later.) To set the axes scales to the same 0-100 limits as before:
\begin{docspecbold}
>> axis([0 100 0 100]);
\end{docspecbold}
...
\noindent Note that here, you are setting line properties \uline{after} you have created the graphics, whereas earlier in eg. using \texttt{plot} to graph data, you specified the properties at the same time as you draw the lines. Both ways are valid, but being able to change properties later and after plotting, gives you greater flexibility. Note that after plotting a graphic, you can also edit and adjust its properties in the \textsf{Figure window} itself (via the \textbf{GUI})
...
\end{docspecbold}
which scatter-plots a single point at location (50,50), it makes the default circle size \texttt{1000} points (3rd parameter), specifies a particular color (\texttt{[0.25 0.75 0.25]}), and then demands that the point  is 'filled'.
...
>> Y1 = [40 60 60 40];
\end{docspecbold}
In this \texttt{(X1,Y1)} point notation, we are telling \textbf{MATLAB} to plot a shape with vertices at:
\vspace{1mm}
\((40,40), (40,60), (60,60), (60,40)\)
\vspace{1mm}

\newpage
To draw the shape, simply pass the 2 coordinate vectors to \texttt{patch}, along with any line style specification:
\begin{docspecbold}
>> patch(X1,Y1,'r');
\end{docspecbold}
...
A \textit{GUI} is only of any real use if it allows some response to input.\sidenote[][-0.0in]{A windows could ne a little error message or warning window, but even then, they typically have an 'OK' button to close them)} This is going to involve a little code of your own ... so we'll start with the simplest possible action -- a \textit{button} that performs a simple action (in this example -- closes the window).
...
Now -- we have the ID of the 2nd text box and we can now set its property (from containing no text, to displaying a suitable message). Let us first implement an answer if the Yes push button is clicked. The command to set a property is ... \docenv{set}. In our example, the \textit{handle} of the text box we have already obtained and assigned to the variable \texttt{h\_answertext}. The name of the property we want to change (refer to the column list in the property editor if you like as a reminder) is \texttt{'String'}.  And the text ... well, you can have whatever you want. The  line is then:
\begin{docspec}
set(h\_answertext,'String','Yes, it is an excellent fruit.');
\end{docspec}

The complete(!) 2-line piece of code in \texttt{\textcolor{blue}{function} yesbutton\_Callback} -- finding the ID of the text box and then setting the text property using that ID -- should then look like:
\begin{docspec}
h\_answertext = findobj('Tag','answertextbox');
\\set(h\_answertext,'String','Yes, it is an excellent fruit.');
\end{docspec}
