...
You can also do this at the the command line ... which is much less fun and you are less likely to need to do it this way. The instructions are included below for completeness.
...
\sidenote[][-3.00in]{This sequence is going to look something like:
\begin{docspecmargin}
>> save MYSTUFF
\\>> exit
\end{docspecmargin}
...
\marginnote[-1.00in]{\begin{mdframed}[backgroundcolor=light-gray, roundcorner=10pt,leftmargin=1, rightmargin=1, innerleftmargin=5, innerrightmargin=5, innertopmargin=5,innerbottommargin=5, outerlinewidth=1, linecolor=light-gray]
\docenvdef{save}
...
\begin{docspec}
>> load('paleo\_CO2\_data.txt','-ascii')
\end{docspec}
...
\marginnote[-4.0in]{\begin{mdframed}[backgroundcolor=light-gray, roundcorner=10pt,leftmargin=1, rightmargin=1, innerleftmargin=5, innerrightmargin=5, innertopmargin=5,innerbottommargin=5, outerlinewidth=1, linecolor=light-gray]
\docenvdef{sortrows}
...
Or , rather crudely, one could plot the min and max values as points, using different symbols. This requires a further little trick in \textbf{MATLAB}, which involves the command \docenv{hold}. This is nice and simple and takes the additional (2nd) word: \texttt{on}, or \texttt{off}.
